...
import com.globallogic.musicplayer.ui.player.notification.NotificationChannelManager.Companion.CHANNEL_ID
	...
	override fun updateAction(notification: Notification, action: Int) {
		when (action) {
			ACTION_PAUSE -> {
				...
				notification.actions[1] = Notification.Action.Builder(Icon.createWithResource(context, R.drawable.ic_play_arrow), context.getString(R.string.play), pendingIntent).build()
			...
			ACTION_PLAY -> {
				...
				notification.actions[1] = Notification.Action.Builder(Icon.createWithResource(context, R.drawable.ic_pause), context.getString(R.string.pause), pendingIntent).build()
